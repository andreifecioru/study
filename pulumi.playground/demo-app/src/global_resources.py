import pulumi as pm
from pulumi_aws import ec2, rds, iam, amplify, elasticbeanstalk as eb
...
def create_public_subnet(vpc, zone, route_table):
    ...
    public_subnet = ec2.Subnet(
        f'{project_name}-public-subnet-{zone}',
        vpc_id=vpc.id,
    ...
    ec2.RouteTableAssociation(
        f'{project_name}-public-rta-{zone}',
...
def create_private_subnet(vpc, zone, route_table):
    ...
    private_subnet = ec2.Subnet(
        f'{project_name}-private-subnet-{zone}',
        vpc_id=vpc.id,
        cidr_block=cidr_block,
        availability_zone=f'{aws_region}{zone}',
    ...
    ec2.RouteTableAssociation(
        f'{project_name}-private-rta-{zone}',
...
def create_bastion_sg(vpc):
    ...
    ec2.SecurityGroupRule(
        f'{project_name}-bastion-all-out',
        type='egress',
        from_port=0,
        to_port=0,
        protocol='-1',
...
def create_instance_sg(vpc, bastion_sg):
    ...
    ec2.SecurityGroupRule(
        f'{project_name}-instance-ssh-in',
        type='ingress',
        from_port=22,
        to_port=22,
        protocol='tcp',
    ...
    # Allow all outbound traffic
    ec2.SecurityGroupRule(
        f'{project_name}-instance-all-out',
        type='egress',
        from_port=0,
        to_port=0,
        protocol='-1',
...
def create_rds_sg(vpc, bastion_sg, instance_sg):
    ...
    # Allow inbound traffic from the bastion host
    ec2.SecurityGroupRule(
        f'{project_name}-rds-postgres-bastion-in',
        type='ingress',
        from_port=5432,
        to_port=5432,
        protocol='tcp',
    ...
    ec2.SecurityGroupRule(
        f'{project_name}-rds-postgres-instance-in',
        type='ingress',
        from_port=5432,
        to_port=5432,
        protocol='tcp',
...
def create_instance_key(vpc):
    ...
    # Create a key-pair for SSH access to instances
    return ec2.KeyPair(
        f'{project_name}-key',
...
def create_amplify_application():
    return amplify.App(
...
def create_global_resources():
    ...
    amplify_app = create_amplify_application()
    ...
    return {
        ...
        'applications.amplify.id': amplify_app.id,
        'applications.amplify.domain': amplify_app.id.apply(lambda app_id: f'{app_id}.amplifyapp.com'),
    }

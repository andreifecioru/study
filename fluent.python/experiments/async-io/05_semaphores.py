...
async def mutate_resource(id: int) -> None:
    ...
    async with semaphore: # grab a semaphore and enter critical section
        print(f'Task {id} acquired a semaphore (value: {shared_resuource}). Remaining permits: {semaphore._value}')
        await asyncio.sleep(delay)
        shared_resuource += 1
        print(f'Task {id} releases the semaphore (value: {shared_resuource}).')

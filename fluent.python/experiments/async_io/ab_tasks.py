...
async def program_01() -> None:
  ...
  t2 = asyncio.create_task(computation(2, 2.0, does_throw=True))
...
async def program_02() -> None:
  tasks = [
    ...
    asyncio.create_task(computation(2, 2.0, does_throw=True)),
...
async def program_03() -> None:
  ...
  async with asyncio.TaskGroup() as tg:
    for idx, sleep_time in enumerate([1.0, 2.0, 0.5], start=1):
      task = tg.create_task(computation(idx, sleep_time, does_throw=(idx == 1)))
      tasks.append(task)
